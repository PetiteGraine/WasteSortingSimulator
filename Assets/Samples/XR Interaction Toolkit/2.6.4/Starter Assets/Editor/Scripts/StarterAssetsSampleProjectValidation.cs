            ...
            if (s_BuildValidationRules.Count == 0)
            {
                ...
                s_BuildValidationRules.Add(
                    new BuildValidationRule
                    {
                        ...
                        Message = $"[{k_StarterAssetsSampleName}] Input System ({k_InputSystemPackageName}) package must be à version {s_RecommendedPackageVersion} or higher to use Project-wide Actions with {k_InputActionAssetName}.",
                        Category = k_Category,
                        CheckPredicate = () => PackageVersionUtility.GetPackageVersion(k_InputSystemPackageName) >= s_RecommendedPackageVersion,
                        FixIt = () =>
                        {
